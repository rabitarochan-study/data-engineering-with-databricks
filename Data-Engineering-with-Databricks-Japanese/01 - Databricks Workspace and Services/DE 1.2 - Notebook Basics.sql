...
-- MAGIC %md
-- MAGIC やっぱり `Scala` でしょ！

-- COMMAND ----------

-- MAGIC %scala
-- MAGIC println("I'm running Scala!")
...
-- MAGIC 
-- MAGIC 
-- MAGIC 参照した **`../Includes/Classroom-Setup-1.2`** ([Link](../Includes/Classroom-Setup-1.2))ノートブックには、データベースを作成し、 **`USE`** するためのロジックとテンポラリビュー **`demo_temp_vw`** を作成するロジックが含まれています。
-- MAGIC 
